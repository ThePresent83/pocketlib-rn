...
func (handler *Handler) requireAdmin(next http.HandlerFunc) http.HandlerFunc {
	return handler.requireRole(next, domain.RoleAdmin)
}

func (handler *Handler) requireStaff(next http.HandlerFunc) http.HandlerFunc {
	return handler.requireRole(next, domain.RoleAdmin, domain.RoleTeacher)
}

func (handler *Handler) requireRole(next http.HandlerFunc, roles ...domain.Role) http.HandlerFunc {
	...
	return handler.requireAuth(func(w http.ResponseWriter, r *http.Request) {
		...
		if !allowed[user.Role] {
			writeError(w, http.StatusForbidden, fmt.Errorf("required role is missing"))
			return
		}
