...
func (service *AuthService) Register(ctx context.Context, input RegisterInput) (AuthResult, error) {
	login, email, fullName, password, err := normalizeCredentials(input.Login, input.Email, input.FullName, input.Password)
	if err != nil {
...
func (service *AuthService) Login(ctx context.Context, input LoginInput) (AuthResult, error) {
	loginOrEmail := strings.TrimSpace(input.Login)
	if loginOrEmail == "" {
		loginOrEmail = strings.TrimSpace(input.Email)
	}
	login, _, _, password, err := normalizeCredentials(loginOrEmail, input.Email, "", input.Password)
	if err != nil {
		return AuthResult{}, err
	}
...
func (service *AuthService) UpdateUser(ctx context.Context, userID string, input UpdateUserInput) (domain.User, error) {
	...
	login, email, fullName, _, err := normalizeCredentials(input.Login, input.Email, input.FullName, "keep")
	if err != nil {
		return domain.User{}, err
	}
...
func normalizeCredentials(login string, email string, fullName string, password string) (string, string, string, string, error) {
...
func (service *AuthService) EnsureDefaultAdmin(ctx context.Context) error {
	login := strings.TrimSpace(service.defaultAdminLogin)
	password := service.defaultAdminPasswd
	if login == "" || password == "" {
		return nil
	}

	if existingUser, err := service.repo.GetUserByLogin(ctx, login); err == nil {
		if existingUser.Role == domain.RoleAdmin {
			return nil
		}
		_, err = service.repo.UpdateUserRole(ctx, domain.UpdateUserRoleInput{
	...
	_, err = service.repo.CreateUserAdmin(ctx, domain.CreateUserAdminInput{
		Login:        login,
		Email:        login,
